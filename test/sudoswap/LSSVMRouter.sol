...
contract LSSVMRouter {
    ...
    function swapNFTsForAnyNFTsThroughETH(
        ...
        outputAmount =
            _swapNFTsForToken(trade.nftToTokenTrades, 0, payable(address(this)));
    ...
    function robustSwapETHForAnyNFTs(
        ...
        for (uint256 i; i < numSwaps;) {
            ...
            (error,,, pairCost,) =
                swapList[i].swapInfo.pair.getBuyNFTQuote(swapList[i].swapInfo.numItems);
            ...
            if (pairCost <= swapList[i].maxCost && error == CurveErrorCodes.Error.OK) {
                ...
                remainingValue -= swapList[i].swapInfo.pair.swapTokenForAnyNFTs{
                    value: pairCost
                }(swapList[i].swapInfo.numItems, pairCost, nftRecipient, true, msg.sender);
            }
    ...
    function robustSwapETHForSpecificNFTsAndNFTsToToken(
        ...
        }
        {
            ...
            for (uint256 i; i < numSwaps;) {
                ...
                {
                    CurveErrorCodes.Error error;
                    (error,,, pairOutput,) = params.nftToTokenTrades[i]
                        .swapInfo
                        .pair
                        .getSellNFTQuote(params.nftToTokenTrades[i].swapInfo.nftIds.length);
                    if (error != CurveErrorCodes.Error.OK) {
                        unchecked {
                            ++i;
                        }
                ...
                if (pairOutput >= params.nftToTokenTrades[i].minOutput) {
                    // Do the swap and update outputAmount with how many tokens we got
                    outputAmount += params.nftToTokenTrades[i]
                        .swapInfo
                        .pair
                        .swapNFTsForToken(
    ...
    function _swapETHForSpecificNFTs(
        ...
        for (uint256 i; i < numSwaps;) {
            // Calculate the cost per swap first to send exact amount of ETH over, saves gas by avoiding the need to send back excess ETH
            (error,,, pairCost,) =
                swapList[i].pair.getBuyNFTQuote(swapList[i].nftIds.length);
    ...
    function _swapNFTsForToken(
        ...
        for (uint256 i; i < numSwaps;) {
            ...
            outputAmount += swapList[i].pair.swapNFTsForToken(
                swapList[i].nftIds, 0, tokenRecipient, true, msg.sender
            );
